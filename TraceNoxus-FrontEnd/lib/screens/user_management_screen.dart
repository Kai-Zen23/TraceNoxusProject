...
class _EditUserDialogState extends State<EditUserDialog> {
  ...
  void initState() {
    ...
    // Parse the game from the user model
    String userGame = "None";
    if (widget.user.gamesPlayed.isNotEmpty) {
      // Get the display name of the first game
      String firstGame = widget.user.gamesPlayed.first.displayName;
      // Try to find a match in the available games list
      try {
        userGame = _availableGames.firstWhere(
          (game) => game.toUpperCase() == firstGame.toUpperCase(),
          orElse: () => "None"
        );
      } catch (e) {
        userGame = "None";
  ...
  Future<void> _saveChanges() async {
    ...
    switch (_selectedGame) {
      ...
      case 'TEKKEN 8':
        ...
        break;
      case 'None':
        // We handle None by sending an empty list of gamesPlayed
        selectedGameEnum = GamePlayed.valorant; // Dummy value, wont be used if we handle list logic correctly
        break;
      default:
        selectedGameEnum = GamePlayed.valorant;
    }
    ...
    final updatedUser = UserManagementModel(
      ...
      role: _selectedRole,
      gamesPlayed: _selectedGame == 'None' ? [] : [selectedGameEnum],
    );

    await context.read<UserManagementProvider>().updateUser(widget.user.id, updatedUser);
    if (mounted) {
